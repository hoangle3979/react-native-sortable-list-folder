...
export default class SortableList extends Component {
  ...
  _renderRows() {
    ...
    const {animated, order, data, activeRowKey, releasedRowKey, rowsLayouts, activeFolder} = this.state;
   ...
   */
  _setOrderOnMove() {
    const {activeRowKey, activeRowIndex, order, data} = this.state;
    ...
    if (rowUnderActiveKey !== activeRowKey && rowUnderActiveKey !== this._prevSwapedRowKey) {
      ...
      if (data[activeRowKey].isFolder) {
        // Khi kéo folder
        if (data[rowUnderActiveIndex].isFolder) {
          if (this.props.folderInFolder) {
            this.setState({
              activeFolder: rowUnderActiveIndex
            }) 
          } else {
            this.setState({
              activeFolder: -1,
  ...
  _onReleaseRow = (rowKey) => {
    ...
    if (this.state.activeFolder > -1) {
      obj = {moveInFolder: this.state.activeFolder}
    }
    ...
    let arrOrder = this.state.order
    if (this.props.data instanceof Array) {
      const arrFolder = this.props.data?.filter((k: any) => k.isFolder)
      if (Number.parseInt(rowKey) > arrFolder.length - 1) {
        arrOrder = arrOrder.slice(arrFolder.length)
        arrOrder = arrOrder.map((k: any) => Number.parseInt(k) - arrFolder.length)
      } else {
        obj = {...obj, sortFolder: true}
        arrOrder = arrOrder.slice(0, arrFolder.length)
      }
    }
    obj = {...obj, rowKey, order: arrOrder}

    if (this.props.onReleaseRow) {
      this.props.onReleaseRow(obj);
    }
  };
